import "server-only";
import JWT from "jsonwebtoken";
import type { Profile, SessionWithProfile } from "@/models/employee";

const { PUBLIC_KEY, PRIVATE_KEY } = process.env;
const TOKEN_EXPIRATION = "24h";

const PUBLIC_KEY_DECODED = Buffer.from(PUBLIC_KEY!, "base64").toString("utf-8");
const PRIVATE_KEY_DECODED = Buffer.from(PRIVATE_KEY!, "base64").toString(
  "utf-8",
);

export interface TokenBody {
  //mail: string;
  id: Uint8Array<ArrayBuffer>;
  username: string;
  iat: number;
  exp: number;
  iss: string;
  sub: string;
  Role_Employee_roleIdToRole: {
    id: Uint8Array<ArrayBuffer>;
    name: string;
    createdAt: Date;
    createdBy: Uint8Array<ArrayBuffer>;
    level: number;
  };
}

export interface StatefulJwtTokenBody extends TokenBody {
  sessionId: Uint8Array<ArrayBuffer>;
}

export const validateJwtToken = (token: string): TokenBody | undefined => {
  try {
    return JWT.verify(token, PUBLIC_KEY_DECODED) as unknown as TokenBody;
  } catch (_e) {
    return undefined;
  }
};

export const validateStatefulJwtToken = (
  token: string,
): StatefulJwtTokenBody | undefined => {
  try {
    return JWT.verify(
      token,
      PUBLIC_KEY_DECODED,
    ) as unknown as StatefulJwtTokenBody;
  } catch (_e) {
    return undefined;
  }
};

export const createJwtToken = (employee: Profile) => {
  return JWT.sign(
    {
      //mail: employee.mail,
      id: employee.id,
      userName: employee.userName,
      Role_Employee_roleIdToRole: employee.Role_Employee_roleIdToRole,
    },
    PRIVATE_KEY_DECODED,
    {
      algorithm: "RS256",
      expiresIn: TOKEN_EXPIRATION,
      subject: employee.userName,
      issuer: "contacts-app",
    },
  );
};

export const createStatefulJwtToken = (session: SessionWithProfile) => {
  return JWT.sign(
    {
      //mail: session.Employee.mail,
      id: session.Employee.id,
      username: session.Employee.userName,
      Role_Employee_roleIdToRole: session.Employee.Role_Employee_roleIdToRole,
      sessionId: session.id,
    },
    PRIVATE_KEY_DECODED,
    {
      algorithm: "RS256",
      expiresIn: TOKEN_EXPIRATION,
      subject: session.Employee.userName,
      issuer: "contacts-app",
    },
  );
};
