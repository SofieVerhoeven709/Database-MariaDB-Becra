 ...
 */
export function protectedFormAction<
  Schema extends ZodType = EmptySchema,
  ReturnType = void,
>(
  options: Omit<
    ServerFunctionOptions<Schema, ReturnType, true>,
    "authenticated"
  >,
): FormAction<ReturnType> {
  return formAction<Schema, ReturnType, true>({
    ...options,
    authenticated: true,
  });
}
 ...
 * @param options An object containing the configuration for the form action.
 */
export function publicFormAction<
  Schema extends ZodType = EmptySchema,
  ReturnType = void,
>(
  options: Omit<
    ServerFunctionOptions<Schema, ReturnType, false>,
    "authenticated" | "requiredRoles"
  >,
): FormAction<ReturnType> {
  return formAction<Schema, ReturnType, false>({
    ...options,
    authenticated: false,
  });
}
...
export function protectedServerFunction(
  options: Omit<
    ServerFunctionOptions<EmptySchema, void, true>,
...
export function publicServerFunction(
  options: Omit<
    ServerFunctionOptions<EmptySchema, void, false>,
    "authenticated" | "requiredRoles" | "schema"
  > & {
    schema?: undefined;
  },
): ServerFunctionWithoutParams;
export function publicServerFunction<Schema extends ZodType>(
  options: Omit<
    ServerFunctionOptions<Schema, void, false>,
    "authenticated" | "requiredRoles"
  >,
): ServerFunction<Schema>;
...
>(
  ...
  try {
    ...
    const submittedData = (
      unvalidatedData instanceof FormData
    ...
    const result = await options.serverFn({ data, profile, logger } as Context<
      ZodType,
      Auth
    >);
    logger.info(
      `${functionName} completed successfully in ${Date.now() - start} ms`,
    );
    return (
      result ?? {
        success: true,
        submittedData: options.sendBackOnSuccess && submittedData,
      }
    );
  } catch (e) {
