...
import {
  createStatefulJwtToken,
  validateStatefulJwtToken,
} from "@/lib/jwtUtils";
...
export async function getSessionProfileFromCookie(
  stateful = true,
...
export async function getSessionProfileFromCookieOrThrow(
  stateful = true,
): Promise<Profile> {
  const session = await getSessionFromCookie(stateful);
  ...
  if (!session) {
    throw new Error(
      "Couldn't retrieve the user's profile in getSessionProfileFromCookieOrThrow.",
    );
  }
...
export async function extendSessionAndSetCookie(
