...
import type { SessionWithProfile } from "@/models/employee";
...
export async function redirectProxy(
  ...
  const parameterizedRoute = request.nextUrl.pathname.replaceAll(
    uuidV4Regex,
    ":param",
  );
  ...
  if (publicRedirects[parameterizedRoute] && session) {
    return NextResponse.redirect(
      new URL(publicRedirects[parameterizedRoute], request.url),
    );
  }
  ...
  if (protectedRoutes.has(parameterizedRoute) && !session) {
    logger.warn(
      `Someone tried to access ${request.nextUrl.pathname} while unauthenticated.`,
    );
  ...
  logger.warn(
    `Granting access to ${request.nextUrl.pathname} because its access level hasn't been configured.`,
  );
