...
import {FunctionFormDialog} from '@/components/custom/functionFormDialog'
import {DepartmentExternFormDialog} from '@/components/custom/departmentExternFormDialog'
...
import {
  ...
  softDeleteFunctionAction,
  hardDeleteFunctionAction,
  undeleteFunctionAction,
} from '@/serverFunctions/functions'
import {
  createDepartmentExternAction,
  updateDepartmentExternAction,
  softDeleteDepartmentExternAction,
  hardDeleteDepartmentExternAction,
  undeleteDepartmentExternAction,
} from '@/serverFunctions/departmentExterns'
...
type ContactTab = 'contacts' | 'invoice' | 'functions' | 'departmentExterns'
...
interface ContactTableProps {
  ...
  initialFunctions: MappedFunctionItem[]
  initialDepartmentExterns: MappedDepartmentExternItem[]
...
export function ContactTable({
  ...
  const [functionFilterDeleted, setFunctionFilterDeleted] = useState<FilterDeleted>('not-deleted')
  const [functionDialogOpen, setFunctionDialogOpen] = useState(false)
  const [editingFunction, setEditingFunction] = useState<MappedFunctionItem | null>(null)

  const [departmentExternSearch, setDepartmentExternSearch] = useState('')
  const [departmentExternFilterDeleted, setDepartmentExternFilterDeleted] = useState<FilterDeleted>('not-deleted')
  const [departmentExternDialogOpen, setDepartmentExternDialogOpen] = useState(false)
  const [editingDepartmentExtern, setEditingDepartmentExtern] = useState<MappedDepartmentExternItem | null>(null)

  const isInvoiceContact = (c: MappedContact) => {
    const role = (c.currentRoleWithCompany ?? '').trim().toLowerCase()
    const lastName = (c.lastName ?? '').trim().toLowerCase()
    return role === 'invoice' || lastName === 'invoice'
  }
  ...
  const normalContacts = baseFiltered.filter(c => !isInvoiceContact(c))
  const invoiceContacts = baseFiltered.filter(isInvoiceContact)
  const visibleContacts = contactTab === 'invoice' ? invoiceContacts : normalContacts

  const filteredFunctions = initialFunctions
    .filter(f => {
      if (functionFilterDeleted === 'not-deleted' && f.deleted) return false
      if (functionFilterDeleted === 'deleted' && !f.deleted) return false
      if (!functionSearch) return true
      return f.name.toLowerCase().includes(functionSearch.toLowerCase())
    })
    .sort((a, b) => a.name.localeCompare(b.name))

  const filteredDepartmentExterns = initialDepartmentExterns
    .filter(d => {
      if (departmentExternFilterDeleted === 'not-deleted' && d.deleted) return false
      if (departmentExternFilterDeleted === 'deleted' && !d.deleted) return false
      if (!departmentExternSearch) return true
      return d.name.toLowerCase().includes(departmentExternSearch.toLowerCase())
    })
    .sort((a, b) => a.name.localeCompare(b.name))
  ...
  async function handleSaveFunction(name: string, id?: string) {
    if (id) {
      ...
      await createFunctionAction({name})
    }
    setFunctionDialogOpen(false)
    router.refresh()
  }

  async function handleSaveDepartmentExtern(name: string, id?: string) {
    if (id) {
      await updateDepartmentExternAction({id, name})
    } else {
      await createDepartmentExternAction({name})
    }
    setDepartmentExternDialogOpen(false)
    router.refresh()
  }
  ...
  const contactsControls = (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <TableCsvActions filename="contact-table.csv" onUpload={handleUploadCsv} />
  ...
  const functionsControls = (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center flex-1">
        ...
        <Select value={functionFilterDeleted} onValueChange={v => setFunctionFilterDeleted(v as FilterDeleted)}>
          <SelectTrigger className="w-[150px] bg-secondary border-border">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="bg-card border-border">
  ...
  const departmentExternControls = (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center flex-1">
        <div className="relative max-w-sm flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input
            placeholder="Search external departments…"
            value={departmentExternSearch}
            onChange={e => setDepartmentExternSearch(e.target.value)}
            className="pl-10 bg-secondary border-border"
          />
        </div>
        <Select
          value={departmentExternFilterDeleted}
          onValueChange={v => setDepartmentExternFilterDeleted(v as FilterDeleted)}>
          <SelectTrigger className="w-[150px] bg-secondary border-border">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="bg-card border-border">
      ...
      {canCreate && (
        <Button
          onClick={() => {
            setEditingDepartmentExtern(null)
            setDepartmentExternDialogOpen(true)
          }}
          className="bg-accent text-accent-foreground hover:bg-accent/80 gap-2">
          <Plus className="h-4 w-4" /> New External Dept
        </Button>
      )}
    </div>
  )
      ...
      <Table>
        <TableHeader>
          <TableRow className="hover:bg-transparent border-border/60">
            ...
            {functionFilterDeleted !== 'not-deleted' && (
              <>
        ...
        <TableBody>
          {filteredFunctions.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={functionFilterDeleted !== 'not-deleted' ? 7 : 4}
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {!f.deleted && canDelete && (
                      <Button
                        ...
                        className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-destructive/10"
                        onClick={async () => {
                          await softDeleteFunctionAction({id: f.id})
                          router.refresh()
                        }}>
                        <Trash2 className="h-3.5 w-3.5" />
                      </Button>
                      ...
                      <>
                        {canDelete && (
                          <Button
                            ...
                            onClick={async () => {
                              await undeleteFunctionAction({id: f.id})
                              router.refresh()
                            }}>
                            Restore
                          </Button>
                        ...
                        {isAdmin && (
                          <Button
                            ...
                            className="h-8 w-8 text-destructive hover:bg-destructive/10"
                            onClick={async () => {
                              await hardDeleteFunctionAction({id: f.id})
                              router.refresh()
                            }}>
                            <Trash2 className="h-3.5 w-3.5" />
                          </Button>
      ...
      <Table>
        <TableHeader>
          <TableRow className="hover:bg-transparent border-border/60">
            ...
            <TableHead className="text-xs whitespace-nowrap">Created At</TableHead>
            <TableHead className="text-xs whitespace-nowrap">Created By</TableHead>
            {departmentExternFilterDeleted !== 'not-deleted' && (
              <>
                <TableHead className="text-xs whitespace-nowrap">Deleted</TableHead>
                <TableHead className="text-xs whitespace-nowrap">Deleted At</TableHead>
                <TableHead className="text-xs whitespace-nowrap">Deleted By</TableHead>
        ...
        <TableBody>
          {filteredDepartmentExterns.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={departmentExternFilterDeleted !== 'not-deleted' ? 7 : 4}
                className="h-32 text-center text-muted-foreground">
                No external departments found.
              </TableCell>
            </TableRow>
          ) : (
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {!d.deleted && canDelete && (
                      <Button
                        ...
                        className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-destructive/10"
                        onClick={async () => {
                          await softDeleteDepartmentExternAction({id: d.id})
                          router.refresh()
                        }}>
                        <Trash2 className="h-3.5 w-3.5" />
                      </Button>
                      ...
                      <>
                        {canDelete && (
                          <Button
                            ...
                            className="h-7 text-xs px-2 text-muted-foreground hover:text-foreground hover:bg-secondary"
                            onClick={async () => {
                              await undeleteDepartmentExternAction({id: d.id})
                              router.refresh()
                            }}>
                            Restore
                          </Button>
                        ...
                        {isAdmin && (
                          <Button
                            ...
                            className="h-8 w-8 text-destructive hover:bg-destructive/10"
                            onClick={async () => {
                              await hardDeleteDepartmentExternAction({id: d.id})
                              router.refresh()
                            }}>
                            <Trash2 className="h-3.5 w-3.5" />
                          </Button>
  ...
  return (
    <div className="flex flex-col gap-6">
      <Tabs value={contactTab} onValueChange={v => setContactTab(v as ContactTab)}>
        <TabsList className="bg-secondary border border-border/60">
          ...
          <TabsTrigger value="invoice">
            Invoice Contacts
            <Badge variant="secondary" className="ml-2 text-xs">
              {invoiceContacts.length}
            </Badge>
          </TabsTrigger>
          <TabsTrigger value="functions">
            Functions
            <Badge variant="secondary" className="ml-2 text-xs">
              {initialFunctions.filter(f => !f.deleted).length}
            </Badge>
          </TabsTrigger>
          <TabsTrigger value="departmentExterns">
            External Departments
            <Badge variant="secondary" className="ml-2 text-xs">
              {initialDepartmentExterns.filter(d => !d.deleted).length}
            </Badge>
          </TabsTrigger>
        </TabsList>
        <TabsContent value="contacts" className="mt-3">
          ...
          <div className="text-xs text-muted-foreground mt-2">
            Showing {visibleContacts.length} of {initialContacts.length} contacts
          </div>
        </TabsContent>
        ...
        <TabsContent value="functions" className="mt-3">
          ...
          {functionsTable}
          <div className="text-xs text-muted-foreground mt-2">
            Showing {filteredFunctions.length} of {initialFunctions.length} functions
          </div>
        </TabsContent>
        <TabsContent value="departmentExterns" className="mt-3">
          {departmentExternControls}
          {departmentExternsTable}
          <div className="text-xs text-muted-foreground mt-2">
            Showing {filteredDepartmentExterns.length} of {initialDepartmentExterns.length} external departments
          </div>
        </TabsContent>
      </Tabs>
      ...
      <DepartmentExternFormDialog
        open={departmentExternDialogOpen}
        onOpenChange={setDepartmentExternDialogOpen}
        item={editingDepartmentExtern}
        onSave={handleSaveDepartmentExtern}
      />
    </div>
  )
}
