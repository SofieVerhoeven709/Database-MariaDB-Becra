...
import {createDepartmentExternAction} from '@/serverFunctions/departmentExterns'

export interface DepartmentExternOption {
  id: string
  name: string
}

interface DepartmentExternSelectProps {
  value: string | null
  currentName?: string | null
  onChange: (id: string | null, name: string | null) => void
  options: DepartmentExternOption[]
  className?: string
}
...
export function DepartmentExternSelect({value, currentName, onChange, options, className}: DepartmentExternSelectProps) {
  function buildOptions(base: DepartmentExternOption[], selectedId: string | null, selectedName: string | null | undefined) {
  ...
  const [list, setList] = useState<DepartmentExternOption[]>(() => buildOptions(options, value, currentName))
  ...
  async function handleCreate() {
    ...
    setError(null)
    try {
      const created = await createDepartmentExternAction({name: trimmed})
      const newOption: DepartmentExternOption = {id: created.id, name: created.name}
      setList(prev => [...prev, newOption].sort((a, b) => a.name.localeCompare(b.name)))
      onChange(created.id, created.name)
      setCreatingMode(false)
      setNewName('')
    } catch {
      setError('Failed to create external department. Please try again.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="flex gap-1.5">
      <Select
        ...
        <SelectTrigger className={`h-8 text-xs bg-secondary border-border flex-1 ${className ?? ''}`}>
          <SelectValue placeholder="Select external department…" />
        </SelectTrigger>
        <SelectContent className="bg-card border-border">
          ...
          <SelectItem value={CREATE} className="text-xs text-accent">
            <span className="flex items-center gap-1">
              <Plus className="h-3 w-3" /> Add new external department…
            </span>
          </SelectItem>
        </SelectContent>
      </Select>
