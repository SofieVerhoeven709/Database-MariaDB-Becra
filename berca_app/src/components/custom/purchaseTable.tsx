...
export function PurchaseTable({
  ...
  const [paymentFilterDeleted, setPaymentFilterDeleted] = useState<'not-deleted' | 'deleted' | 'all'>('not-deleted')
  ...
  const filtered = initialPurchases
    .filter(p => {
      ...
      if (paymentConditionFilter === PAYMENT_FILTER_NONE && p.paymentConditionId !== null) return false
      if (paymentConditionFilter !== PAYMENT_FILTER_ALL && paymentConditionFilter !== PAYMENT_FILTER_NONE) {
        if (p.paymentConditionId !== paymentConditionFilter) return false
      }
      ...
      return (
        ...
        (p.poNumberClient ?? '').toLowerCase().includes(q) ||
  ...
  async function handleSave(p: MappedPurchase) {
    if (editing) {
      await updatePurchaseAction({
        ...
        poNumberClient: p.poNumberClient,
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center flex-1">
              ...
              <Select value={paymentConditionFilter} onValueChange={setPaymentConditionFilter}>
                <SelectTrigger className="w-[200px] bg-secondary border-border">
                  <SelectValue placeholder="Payment condition" />
                </SelectTrigger>
                <SelectContent className="bg-card border-border">
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map(purchase => {
                    const secondaryLabel = purchase.poNumberClient ?? purchase.paymentConditionName ?? ''
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                            ...
                            <Button
                              ...
                              size="icon"
                              className="h-7 w-7 text-muted-foreground hover:text-foreground hover:bg-secondary"
                              disabled={!canEdit || !canMutatePurchase}
                              onClick={() => {
                                setEditing(purchase)
                                setDialogOpen(true)
                              }}>
                            ...
                            <Button
                              ...
                              disabled={!canDelete || !canMutatePurchase}
        ...
        <TabsContent value="payment-conditions" className="mt-4 space-y-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center flex-1">
              ...
              <Select
                value={paymentFilterDeleted}
                onValueChange={v => setPaymentFilterDeleted(v as 'not-deleted' | 'deleted' | 'all')}>
