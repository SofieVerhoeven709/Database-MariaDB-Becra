import type {MappedBoq, MappedBoqContact, MappedBoqWorkOrder, MappedBillingLine} from '@/types/billOfQuantity'
...
function findStayOverPrice(items: PriceListItemRaw[]): {itemId: string; unit: string; basePrice: number} | null {
  ...
  if (byUnit) return {itemId: byUnit.id, unit: byUnit.unit, basePrice: byUnit.price.toNumber()}

  const byDescription = items.find(item => {
    if (item.isCostMargin) return false
    return normalizeText(item.description).replace(/_/g, '') === 'STAYOVER'
  })
  if (!byDescription) return null
  return {itemId: byDescription.id, unit: byDescription.unit, basePrice: byDescription.price.toNumber()}
}
...
type BoqRaw = {
  ...
  BillOfQuantitiesType: {id: string; name: string}
  Employee_BillOfQuantities_createdByToEmployee: {id: string; firstName: string; lastName: string}
  PaymentMethod: {id: string; name: string}
  BillOfQuantitiesSentType: {id: string; name: string}
  BillOfQuantitiesStatus: {id: string; name: string}
  PriceList: {
    id: string
    name: string
...
function mapWorkOrderWithLines(
  ...
  for (const tr of wo.TimeRegistry) {
    if (tr.invoiceTime !== true) continue
  ...
  for (const [hourTypeId, group] of Array.from(hoursByType.entries()).sort((a, b) =>
  ...
  for (const [hourTypeId, group] of Array.from(stayOverByType.entries()).sort((a, b) =>
    a[1].label.localeCompare(b[1].label),
  )) {
...
export function mapBoq(r: BoqRaw, materialPriceMap: Map<string, number> = new Map()): MappedBoq {
  ...
  for (const wo of workOrders) {
    for (const {rate, amount} of wo.vatByRate) {
      boqVatByRateMap.set(rate, (boqVatByRateMap.get(rate) ?? 0) + amount)
    }
  }
  ...
  const totalInclVat = subtotalExVat + totalVat
  const vatByRateArray = Array.from(boqVatByRateMap.entries())
    .map(([rate, amount]) => ({rate, amount}))
    .sort((a, b) => a.rate - b.rate)
  ...
  return {
    ...
    createdByName: `${r.Employee_BillOfQuantities_createdByToEmployee.firstName} ${r.Employee_BillOfQuantities_createdByToEmployee.lastName}`,
    modifiedBy: r.modifiedBy,
    modifiedByName: null,
    boqTypeId: r.boqTypeId,
    boqTypeName: r.BillOfQuantitiesType.name,
    targetId: r.targetId,
    paymentMethodId: r.paymentMethodId,
    paymentMethodName: r.PaymentMethod.name,
    boqSentTypeId: r.boqSentTypeId,
    boqSentTypeName: r.BillOfQuantitiesSentType.name,
    boqStatusId: r.boqStatusId,
    boqStatusName: r.BillOfQuantitiesStatus.name,
    priceListId: r.priceListId,
    priceListName: r.PriceList?.name ?? null,
    contacts: r.BoqContact.map(
      (c): MappedBoqContact => ({
        id: c.id,
