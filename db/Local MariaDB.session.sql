        ...
        CREATE TABLE
            IF NOT EXISTS Purchase (
                ...
                bocCustomerName VARCHAR(255),
                bocDescription TEXT,
                bocCreatedAt DATETIME,
                bocStatus VARCHAR(50),
                bocNumber VARCHAR(255),
                description TEXT,
                additionalInfo VARCHAR(255),
        ...
        CREATE
        OR REPLACE VIEW vw_materialSerialTrackInspectionAlerts AS
        SELECT
            mst.id,
            mst.materialId,
            mst.beNumber,
            ...
            (
                DATEDIFF (mst.nextInspectionDate, CURDATE ()) BETWEEN 0 AND 30
            ) AS remindMonthBefore,
            (
                DATEDIFF (mst.nextInspectionDate, CURDATE ()) BETWEEN 0 AND 7
            ) AS remindWeekBefore,
            (
                DATEDIFF (mst.nextInspectionDate, CURDATE ()) BETWEEN 0 AND 1
            ) AS remindDayBefore,
            (DATEDIFF (mst.nextInspectionDate, CURDATE ()) < 0) AS isOverdue
        FROM
            MaterialSerialTrack mst
        WHERE
            mst.deleted = 0
            AND mst.nextInspectionDate IS NOT NULL;

        CREATE
        OR REPLACE VIEW vw_materialSerialTrackDueCurrentMonth AS
        SELECT
            mst.id,
            mst.materialId,
            mst.beNumber,
    ...
    ADD COLUMN IF NOT EXISTS weeklyWorkHours DECIMAL(5,2) NOT NULL DEFAULT 40.00,
    ADD COLUMN IF NOT EXISTS workScheduleType VARCHAR(20) NOT NULL DEFAULT 'fixed',
    ADD COLUMN IF NOT EXISTS overtimeTrackingEnabled BOOLEAN NOT NULL DEFAULT 0,
    ADD COLUMN IF NOT EXISTS maxOvertimeHours DECIMAL(7,2) NULL;

    ALTER TABLE HrCertificationTraining
    ADD COLUMN IF NOT EXISTS trainingDocumentNumber VARCHAR(100) NULL,
    ADD COLUMN IF NOT EXISTS includeInHseFile BOOLEAN NOT NULL DEFAULT 0;
